<div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-10">
                    <div class="section_title text-center mb-55">

                    <h2>Lakukan Percepatan dalam Belajar Coding</h2>
                        Media belajar online yang akan membantumu mengembangkan keahlianmu dalam bidang pemrograman
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <div class="causes_active owl-carousel">
                        <div class="single_cause">                     
                            <div class="causes_content" style="text-align: center">
                                <div class="thumb" style="font-size: 6rem;">
                                <i class="fas fa-laptop-code"></i>                            
                                </div>
                                <h4>Pelajari cara menggunakan StackOverflow dan Github</h4>
                                {{-- <p>The passage is attributed to an
                                    unknown typesetter in the century
                                    who is thought</p>
                                <a class="read_more" href="cause_details.html">Read More</a> --}}
                            </div>
                        </div>
                        <div class="single_cause">                     
                            <div class="causes_content" style="text-align: center">
                                <div class="thumb" style="font-size: 6rem;">
                                <i class="fab fa-codepen"></i>                            
                                </div>
                                <h4>Latihlah dirimu menjadi programmer yang otodidak</h4>
                                {{-- <p>The passage is attributed to an
                                    unknown typesetter in the century
                                    who is thought</p>
                                <a class="read_more" href="cause_details.html">Read More</a> --}}
                            </div>
                        </div>
                        <div class="single_cause">                     
                            <div class="causes_content" style="text-align: center">
                                <div class="thumb" style="font-size: 6rem;">
                                    <i class="fas fa-microphone-alt"></i>                                </div>
                                <h4>Seringlah hadiri pertemuan, wawancara, dan hackathon</h4>
                                {{-- <p>The passage is attributed to an
                                    unknown typesetter in the century
                                    who is thought</p>
                                <a class="read_more" href="cause_details.html">Read More</a> --}}
                            </div>
                        </div>
                        
                    </div>
                </div>
            </div>
        </div>